...
acvm_t *acvm_set_command_set(acvm_t *p, int com_size, char *temp_command)
{
...
acvm_t *acvm_set_array(acvm_t *p, int arr_size, char *temp_array)
{
	int no_array = 0;
	int buffer = 0;
...
int main()
{
	...
	for(int i = 0; i < rpt; i++)
	{
		...
		test_case[i] = acvm_set_command_set(test_case[i], size_command, temp_command);
		test_case[i] = acvm_set_array(test_case[i], size_array, temp_array);
		...
		for(int j = 0; j < test_case[i]->array_size; j++)
		{
			if(test_case[i]->read_direction == 0)
			{
				if(test_case[i]->array[j] != 0)
				{
					...
					if(j + 1 != test_case[i]->array_size && test_case[i]->array[j + 1] != 0) printf(",");
				}
			}
			else
			{
				if(test_case[i]->array[test_case[i]->array_size - 1 - j] != 0)
				{
					...
					if(j + 1 != test_case[i]->array_size && test_case[i]->array[test_case[i]->array_size - 2 - j] != 0) printf(",");
				}
			}
